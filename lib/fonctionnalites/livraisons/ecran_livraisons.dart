...
class _EcranLivraisonsState extends State<EcranLivraisons> {
  ...
  Future<List<Map<String, dynamic>>> _chargerLivraisons(String userId) async {
    final response = await _client
        .from('livraisons')
        .select('id, statut, prix_livraison, commission, created_at, depart_texte, arrivee_texte, client_id, vendeur_id, livreur_id')
        .or('client_id.eq.$userId,vendeur_id.eq.$userId,livreur_id.eq.$userId')
...
class _BadgeStatut extends StatelessWidget {
  ...
  (Color, Color) _style(String value, ThemeData theme) {
    switch (value) {
      case 'livree':
        return (Colors.green.withOpacity(0.16), Colors.green[800]!);
      case 'annulee':
        return (Colors.red.withOpacity(0.14), Colors.red[800]!);
      case 'en_cours':
        return (Colors.blue.withOpacity(0.14), Colors.blue[800]!);
      case 'acceptee':
        return (theme.colorScheme.primary.withOpacity(0.16), theme.colorScheme.primary);
      case 'en_attente':
      default:
        return (Colors.orange.withOpacity(0.18), Colors.orange[900]!);
    }
  }
